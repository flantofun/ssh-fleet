...
- ❤️ **MIT 协议**，TypeScript 编写，附 32 个单元测试
...
Docker 演示环境会在本机启动两台一次性 SSH 主机，无需云账号或现有基础设施即可验证完整流程。
...
在选定主机上执行本地多行 shell 脚本，无需先把脚本上传到服务器。支持与 `exec`
相同的并发、筛选、超时和输出参数。
...
SSH Fleet 的 Build Week 版本由 Codex 使用 GPT-5.6 协助开发。Codex 审查了命令架构，
实现多行脚本 `run` 工作流，加强参数校验，补充测试，并整理可复现的评审体验。
具体实现记录及验证结果见 [CODEX_BUILD_LOG.md](CODEX_BUILD_LOG.md)。
